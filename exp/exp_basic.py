...
class Exp_Basic(object):
    ...
    def load_state_dict(self, state_dict: typing.OrderedDict[str, torch.Tensor], model=None, strict=False) -> nn.Module:
        ...
        # Remove keys starting with 'expert_gate' from state_dict (or its 'model' subdict)
        
        if 'model' in state_dict:
            keys_to_remove = [k for k in state_dict['model'].keys() if k.startswith('expert_gate')]
            for k in keys_to_remove:
                state_dict['model'].pop(k)
        else:
            keys_to_remove = [k for k in state_dict.keys() if k.startswith('expert_gate')]
            for k in keys_to_remove:
                state_dict.pop(k)

        if self.args.online_method == 'DynaME':
            strict = False
