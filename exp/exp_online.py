...
from util.tools import test_params_flop, log_forecast_to_wandb
    ...
    def online(self, online_data=None, target_variate=None, phase='test', show_progress=False):
        ...
        if online_data is None:
            online_data = get_dataset(self.args, phase, self.device,
                                      wrap_class=self.args.wrap_data_class + [Dataset_Recent],
                                      **self.wrap_data_kwargs, past_num=self.args.past_num)
        ...
        if phase == 'test' and self.args.wandb:
            name = self.args.model+'_'+self.args.dataset+'_'+str(self.args.pred_len)+'_'+str(self.args.online_method)
            if self.args.online_method in ['DynaME']:
                name += '_beta'+str(self.args.beta)+'_delta'+str(self.args.delta)+'_period'+str(self.args.period_num)+'_n'+str(self.args.krr_train_num)+self.args.wandb_postfix

            wandb.init(project="OTS", name=name, config=vars(self.args))
        ...
        for i, (recent_data, current_data) in enumerate(online_loader):
            ...
            self.model.eval()
            with torch.no_grad():
                if self.args.plot_expert and phase == 'test':
                    outputss = self.forward(current_data)
                    outputs = outputss[:-1]
                    expert_preds = outputss[-1]
                else:
                    outputs = self.forward(current_data)
 

                if (i % self.args.plot_freq) == 0 and self.args.wandb and self.args.plot_pred and phase == 'test':
                    lookback_data = current_data[0][0, :, 0].cpu().numpy()
                    ground_truth = current_data[1][0, :, 0].cpu().numpy()
                    if self.args.online_method in ['DynaME']:
                        prediction = outputs[0][0, :, 0].cpu().numpy()
                    else:
                        prediction = outputs[0, :, 0].cpu().numpy()
                    log_forecast_to_wandb(lookback_data, ground_truth, prediction, step=i, chart_name="Forecast Plot")

                    if self.args.plot_expert:
                        for iii in range(self.args.period_num):
                            expert_pred = expert_preds[iii][0, :, 0].cpu().numpy()
                            log_forecast_to_wandb(lookback_data, ground_truth, expert_pred, step=i, chart_name=f"Expert {iii} Forecast Plot")
                ...
                if self.args.analysis:
                    mmse = F.mse_loss(outputs, current_data[self.label_position].to(self.device), reduction='none').detach().squeeze().mean(dim=0)
                    mse_record.append(mmse)

                if self.args.wandb and phase == 'test':
                    if self.args.online_method in ['DynaME']:
                        preds, _, expert_weights, ss = outputs
                        expert_weights_list = expert_weights[:, 0, :].squeeze().detach().cpu().numpy().tolist()
                        log_dict = {f"expert_{idx}_weight": w for idx, w in enumerate(expert_weights_list)}

                        wandb.log(log_dict, step=i)
                        std_mean = ss#ss.squeeze().cpu().numpy().mean()
                        log_dict = {f"expert_std": std_mean}
                        wandb.log(log_dict, step=i)
                    ...
                    current_mse = F.mse_loss(preds, current_data[self.label_position].to(self.device))
                    wandb.log({"step_MSE": current_mse.item()}, step=i)
                    metrics = calculate_metrics(statistics)
                    wandb.log({"cumMSE": metrics['MSE'], "cumMAE": metrics['MAE']}, step=i)
        ...
        if self.args.analysis:
            st()
            records = torch.stack(mse_record).cpu().numpy()
            np.save('./proceed', records)
