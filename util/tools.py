...
def log_forecast_to_wandb(lookback_data, ground_truth, prediction, step, chart_name="Forecast Plot"):
    """
    시계열 예측 결과를 Matplotlib으로 그리고 wandb에 로깅합니다.

    Args:
        lookback_data (np.array): 예측에 사용된 과거 데이터 (lookback window)
        ground_truth (np.array): 예측 구간의 실제 값
        prediction (np.array): 모델이 예측한 값
        chart_name (str): wandb에 기록될 차트의 이름
    """
    # 1. Figure 및 축 생성
    fig, ax = plt.subplots(figsize=(12, 6))

    # 2. x축 인덱스 설정
    lookback_len = len(lookback_data)
    horizon_len = len(ground_truth)
    
    # 전체 기간 (lookback + horizon)의 x축
    x_total = np.arange(lookback_len + horizon_len)
    # 예측 기간의 x축
    x_pred = np.arange(lookback_len, lookback_len + horizon_len)

    # 3. 파란 선 그리기 (Lookback + Ground Truth)
    # lookback 데이터와 실제값을 연결하여 전체 시계열 생성
    blue_line_data = np.concatenate([lookback_data, ground_truth])
    ax.plot(x_total, blue_line_data, color='blue', label='Ground Truth (Lookback + Actual)')

    # 4. 주황색 선 그리기 (Prediction)
    ax.plot(x_pred, prediction, color='orange', label='Prediction', linewidth=2)
    
    # 5. 그래프 꾸미기
    # lookback과 예측 구간을 나누는 점선 추가
    ax.axvline(x=lookback_len - 1, color='gray', linestyle='--', linewidth=1)
    ...
    # 6. wandb에 Figure 로깅
    wandb.log({chart_name: fig}, step=step)
    
    # 7. 메모리 관리를 위해 plot 닫기
    plt.close(fig)
