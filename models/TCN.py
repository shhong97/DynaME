...
class Model(nn.Module):
    def __init__(self, args):
        ...
        # Channelwise regression - separate prediction for each channel
        self.regressor = nn.Linear(320, self.pred_len * self.c_out)
    ...
    def forward(self, x, x_mark=None, return_emb=False):
        ...
        y = y.reshape(len(y), self.pred_len, self.c_out)
        if return_emb:
            return y, rep
        else:
            return y
