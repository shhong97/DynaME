...
parser.add_argument('--dataset', type=str, default='ETTm1', help='data file')
...
parser.add_argument('--beta_xpatch', type=float, default=0.3, help='beta')
...
# DynaME
parser.add_argument('--krr_lambda', type=float, default=0.0001)
parser.add_argument('--krr_train_num', type=int, default=8)
parser.add_argument('--head_learning_rate', type=float, default=0.01)
parser.add_argument('--wandb', action='store_true', default=False)
parser.add_argument('--krr_temperature', type=float, default=1.0)

parser.add_argument('--repeat', action='store_true', default=False) # navie basline

parser.add_argument('--plot_pred', action='store_true', default=False)
parser.add_argument('--plot_expert', action='store_true', default=False)
parser.add_argument('--past_num', type=int, default=672)
parser.add_argument('--balance_loss_type', type=str, default='mse') 
parser.add_argument('--weight_type', type=int, default=0) # 0 :head, 1 : param, 2: 1-layer head, 3: 1-layer direct input head
parser.add_argument('--period_num', type=int, default=2)
parser.add_argument('--alpha', type=float, default=0.5)
parser.add_argument('--beta', type=float, default=0.3)
parser.add_argument('--delta', type=float, default=0.01)
parser.add_argument('--clip_expert', action='store_true', default=False)
parser.add_argument('--plot_std', action='store_true', default=False)
parser.add_argument('--generate_checkpoint', action='store_true', default=False)

parser.add_argument('--plot_freq', type=int, default=100)
parser.add_argument('--analysis', action='store_true', default=False)
parser.add_argument('--wandb_postfix', type=str, default='')
parser.add_argument('--fixed_period', action='store_true', default=False)

args = parser.parse_args()


if args.weight_type == 1:
    args.wandb_postfix = 'learn'
elif args.weight_type == 3:
    args.wandb_postfix = 'distinct'
elif args.weight_type == 4:
    args.wandb_postfix = 'avg'

if args.fixed_period:
    args.wandb_postfix += '_fixed'
    args.period_num = 2

if args.plot_expert:
    args.wandb_postfix += '_plotexpert'
...
if __name__ == '__main__':
    ...
    if isinstance(exp, Exp_SOLID) and args.wandb:
        name = f"{args.model}_{args.dataset}_{args.pred_len}_SOLID"
        wandb.init(project="OTS", name=name, config=vars(args))
        wandb.log({'mse': all_results['mse'][0], 'mae': all_results['mae'][0]})
        wandb.finish()
